...
const ValidationPanel: React.FC<ValidationPanelProps> = ({ validationResult, onNavigateToError }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      {/* Scrollable Content - includes overview and errors/warnings */}
                ...
                >
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 break-words overflow-wrap-anywhere">{error.message}</p>
                      <div className="mt-1 flex items-center space-x-2 text-xs text-gray-500 flex-wrap gap-1">
                        ...
                        {hasLocation && (
                          <span className={`flex items-center space-x-1 ${onNavigateToError ? 'text-blue-600 hover:text-blue-800' : ''} flex-shrink-0`}>
                            <MapPin className="w-3 h-3 flex-shrink-0" />
                            <span className="whitespace-nowrap">Line {error.location!.start_line_number}, Col {error.location!.start_line_character_number}</span>
                            {onNavigateToError && (
                              <span className="text-blue-500 ml-1 whitespace-nowrap">(click to navigate)</span>
                            )}
                          </span>
                        )}
                      </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 break-words overflow-wrap-anywhere">{warning.message}</p>
                      <div className="mt-1 flex items-center space-x-2 text-xs text-gray-500 flex-wrap gap-1">
                        ...
                        {hasLocation && (
                          <span className={`flex items-center space-x-1 ${onNavigateToError ? 'text-blue-600 hover:text-blue-800' : ''} flex-shrink-0`}>
                            <MapPin className="w-3 h-3 flex-shrink-0" />
                            <span className="whitespace-nowrap">Line {warning.location!.start_line_number}, Col {warning.location!.start_line_character_number}</span>
                            {onNavigateToError && (
                              <span className="text-blue-500 ml-1 whitespace-nowrap">(click to navigate)</span>
                            )}
                          </span>
              ...
              <div key={index} className="p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-1 break-words overflow-wrap-anywhere">{suggestion.reason}</p>
                  </div>
                </div>
              </div>
            ))}
