  ...
  const mutation = useMutation<ValidationResult, Error, string>({
    mutationFn: async (jsonText: string) => {
      ...
      try {
        ...
        console.log('🎯 JSON Hook mutationFn: Error occurred:', error)
        ...
        if (error.details?.detail?.validation_result) {
          console.log('🎯 JSON Hook mutationFn: Returning validation result from error.details.detail')
          return error.details.detail.validation_result
        }
